  ...
  const scrapeVisibleResults = () => {
    ...
    return links.map((link) => {
      ...
      if (roleImgContainer) {
        ...
        if (ariaLabel) {
          const match = ariaLabel.match(/([\d,.]+)\s*bintang\s+(\d+)/i);
          if (match) {
            rating = match[1].replace(",", "."); // ganti koma jadi titik agar jadi angka valid (3.5)
            reviewCount = match[2];
          }
        }
      }
      ...
      const phoneRegex = /(?:\+62|62|0)\d{1,3}[\s.-]?\d{3,4}[\s.-]?\d{3,5}/;
